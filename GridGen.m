function [ grid ] = GridGen( nx, ny , r)
%
% Grid generator used for the take-home final exam of AA214
% Written by Benjamin Blake, modified by Faisal As'ad
%
% Input parameters:
%   nx - the number of cells in the x direction
%   ny - the number of cells in the y direction
%   r  - the ratio deltax_outlet/deltax_inlet
%
% Output parameters:
...
%get linearly progressing increment for x grid
xlength = 4;
...
for i=1:(nx+1)
    if (grid.x(i,1) >= 0.5)
        grid.y(i,1) = grid.y(i,1) + (grid.x(i,1) - 0.5) * tan(ramp*pi/180);
    end
end
for i=1:nx+1
    dy = grid.y(i,1);
