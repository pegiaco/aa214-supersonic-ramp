function [V, t_final, criterion_array, wall_clock_time] = method_1_local_time_step(delta_t_global, grid, eps, cfl, name)
...
delta_x_min = min(min(grid.edge(:,:,2), [], "all"), min(grid.edge(:,:,4), [], "all"));
...
figure(1);
plot_results(V, 0, grid, [], "Local time-step", 1, name, 0, cfl);
close(figure(1));
...
while conv_criterion > eps
    ...
    for j = 1:ny
        for i = 1:nx
            ...
            % local time-step
            delta_t_local = delta_t_global*min(grid.edge(i,j,2), grid.edge(i,j,4))/delta_x_min;
            
            % update W
            W(i,j,:) = real(squeeze(W_current(i,j,:)) - (delta_t_local/grid.area(i,j))*sum(F,2));
    ...
    % plot
    figure(n);
    plot_results(V, n*delta_t_global, grid, criterion_array, "Local time-step", 1, name, n, cfl);
    close(figure(n));
